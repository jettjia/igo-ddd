...
## 层级说明
```base
├── api 接入层 【http/grpc/mcp适配, 这一层调用application层; 以使用gin提供http接口为例，这里的handler等为使用gin提供的一些http接口】
│   ├── grpc
│   └── http
│   ├── event 事件
│   │   └── subscribe mq消费入口
│   ├── job 定时任务
│   ├── mcp mcp接口
├── application 应用层 【主要是调用domain层与infra层来实现功能】
...
└── infra 基础设施层 【这里提供了针对domain层的repository接口的实现，还有其他一些基础的组件，提供给application层或者api层使用】
...
│   │   └── po 数据库映射对象
│   └──  external 外部服务集成
└── types 完全独立的模块(DP)，封装自定义的参数类型,例如 phone 相关的类型，校验合法、区号等；常量定义
│   └── apierror 错误码定义
│   └── consts 常量
└── boot 系统启动的初始化脚本
└── config 系统依赖的配置文件
